...
**Applies To:** CeloHT and its affiliated repositories under the current Celo-HaiTi organization
...
```mermaid
flowchart LR
    subgraph Social["Social Impact Activities (Free / Mission-Funded)"]
        A1[Free Web3 & Financial Literacy Education]
        A2[Community Workshops]
        A3[Wallet & Agent Onboarding Support]
        A4[Open-Source Software & Documentation]
        A5[Community Governance]
        A6[Reforestation Programs]
    end

    subgraph Revenue["Revenue-Generating Activities (Fund the Mission)"]
        B1[Professional Certifications]
        B2[Enterprise & Consulting Services]
        B3[Merchant Business Tools]
        B4[Developer Platform & APIs]
        B5[Agent Network Premium Services]
        B6[Impact & ESG Reporting Services]
        B7[Partnerships, Sponsorships, Grants]
    end

    Social -- builds trust, community, pipeline --> Revenue
    Revenue -- funds and scales --> Social
```
...
| **Key Resources** | Open-source codebase (Apache 2.0); brand and documentation assets; trained agent network; Governance Council governance capacity; Celo blockchain infrastructure |
...
```mermaid
flowchart TD
    Celo[Celo Blockchain Infrastructure] --> CeloHT[CeloHT Core Organization]
    CeloHT --> Edu[Education Pillar]
    CeloHT --> Agent[Agent Network Pillar]
    CeloHT --> Reforest[Reforestation Pillar]

    Edu --> Students[Students / Youth]
    Edu --> Enterprises[Enterprise Training Clients]
    Edu --> Universities[University Partners]

    Agent --> LocalAgents[Certified Local Agents]
    LocalAgents --> Merchants[Small Businesses / Merchants]
    LocalAgents --> Diaspora[Diaspora Remittance Users]

    Reforest --> Farmers[Farmers / Landholders]
    Reforest --> ImpactReports[Impact & ESG Reporting Clients]

    CeloHT --> Governance[Founder / Governance Council / Contributors]
...
| Governance Council / contributors | Sustainable, well-governed organization | Diversified funding reducing single-point dependency |
...
- Contributors (open-source volunteers, translators, local organizers)
...
```mermaid
flowchart LR
    CeloHT((CeloHT)) --- CeloFoundation[Celo Ecosystem / Grant Programs]
    CeloHT --- NGOs[NGOs & Cooperatives]
    CeloHT --- Univ[Universities]
    CeloHT --- DevAgencies[Development Agencies]
    CeloHT --- Strategic[Strategic Collaborators - e.g., prospective environmental partners]
    CeloHT --- Enterprises[Enterprise Clients]
    CeloHT --- OSSCommunity[Open-Source Community]
```

Illustrative partner category (prospective, unconfirmed): environmental/community-services initiatives such as FreClean MAY, subject to formal due diligence and Governance Council approval, become Strategic Collaborators on reforestation-adjacent programming. No such agreement is asserted as active by this document.
...
```mermaid
flowchart TD
    Grants[Grants & Donations] --> Pool[CeloHT Operating Pool]
    Service[Service Revenue] --> Pool
    Sponsor[Sponsorships & Partnerships] --> Pool
    Pool --> FreeEdu[Free Education & Community Programs]
    Pool --> AgentSupport[Agent Network Support]
    Pool --> Reforest[Reforestation Programs]
    Pool --> Ops[Core Operations & Governance]
    Pool --> Reinvest[Reinvestment in Revenue-Generating Services]
    Reinvest --> Service
```
...
```mermaid
flowchart LR
    Input[Inputs: funding, contributors, open-source infrastructure] --> Activity[Activities: education, agent onboarding, reforestation]
    Activity --> Output[Outputs: participants trained, agents certified, trees planted]
    Output --> Outcome[Outcomes: increased financial capability, income, environmental resilience]
    Outcome --> Impact[Impact: broader community financial inclusion and ecological resilience]
```
...
```mermaid
flowchart LR
    Education[Education] --> Community[Community Growth]
    Community --> Wallet[Wallet Adoption]
    Wallet --> Merchant[Merchant Adoption]
    Merchant --> Transactions[Transaction Volume]
    Transactions --> Revenue[Revenue - Agent & Merchant Services]
    Revenue --> Partnerships[Partnerships & Reinvestment]
...
4. **Governance scaling:** As activity grows, the Governance Council SHOULD expand deliberately (see `GOVERNANCE.md`) to avoid concentration of decision-making even as the organization grows.
...
4. CeloHT SHOULD maintain a prudent operating reserve (target to be set by the Governance Council once Phase 2 financial data exists) sufficient to sustain core mission activities through short-term funding gaps.
...
- Should be read alongside `PARTNERSHIPS.md`, `GOVERNANCE.md`, and (where published) `LEGAL_STATUS.md` and `FUND_ALLOCATION_FRAMEWORK.md` for full compliance context.
...
1. Any new revenue stream or material pricing change MUST be reviewed by the Governance Council before launch.
2. The Founder MUST NOT unilaterally approve new commercial offerings; approval follows the same community-governance logic as partnerships (see `PARTNERSHIPS.md` Section 22).
...
- [`GOVERNANCE.md`](./GOVERNANCE.md) - Founder, Governance Council, and Community Contributor structure
...
- `FUND_ALLOCATION_FRAMEWORK.md` - *(planned)* Grant and funding acceptance criteria
- `LEGAL_STATUS.md` - *(planned)* CeloHT's legal structure and jurisdictional status
- `FINANCIAL_TRANSPARENCY.md` - *(planned)* Public reporting commitments
