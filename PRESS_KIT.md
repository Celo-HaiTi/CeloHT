# CeloHT Press Kit

**Resources for Journalists and Media Organizations**

This Press Kit provides media-ready resources for reporting on CeloHT. For complete organizational background, brand identity specifications, governance detail, and community impact information, refer to the companion document, MEDIA_KIT.md.

---

## Table of Contents

1. [How to Use This Press Kit](#how-to-use-this-press-kit)
2. [Official Press Release Template](#official-press-release-template)
3. [Project Fact Sheet](#project-fact-sheet)
4. [Company Boilerplate](#company-boilerplate)
5. [Founder Biography](#founder-biography)
6. [Leadership Profiles](#leadership-profiles)
7. [Official Quotes](#official-quotes)
8. [Key Messages](#key-messages)
9. [Media FAQs](#media-faqs)
10. [Press Photos](#press-photos)
11. [Logo Downloads](#logo-downloads)
12. [Brand Assets](#brand-assets)
13. [Screenshots and Product Images](#screenshots-and-product-images)
14. [Event Photos](#event-photos)
15. [Media Contact Information](#media-contact-information)
16. [Interview Guidelines](#interview-guidelines)
17. [Press Coverage Guidelines](#press-coverage-guidelines)
18. [Speaking Topics](#speaking-topics)
19. [News Announcements](#news-announcements)
20. [Awards and Recognition](#awards-and-recognition)
21. [Downloadable Resources](#downloadable-resources)
22. [Citation Guidelines](#citation-guidelines)
23. [Attribution Requirements](#attribution-requirements)
24. [Usage Permissions](#usage-permissions)
25. [Copyright](#copyright)
26. [Legal Disclaimer](#legal-disclaimer)

---

## How to Use This Press Kit

This document is intended for journalists, editors, and news organizations preparing coverage of CeloHT. It provides ready-to-use factual material, quotes, and asset references. For extended background on mission, governance, brand system, and community programs, see MEDIA_KIT.md, which this Press Kit is designed to complement rather than repeat.

---

## Official Press Release Template

```
FOR IMMEDIATE RELEASE

[Headline: Concise, factual, no promotional language]

[City, Country - Date] - CeloHT, a community-governed Web3 initiative
headquartered in Léogâne, Haiti, today announced [announcement summary
in one sentence].

[Paragraph 1 - What happened, who is involved, why it matters. Lead
with the most newsworthy fact.]

[Paragraph 2 - Supporting detail: relevant program (Education, Agent
Network, or Reforestation), scope, and context within CeloHT's mission
of financial inclusion, blockchain education, and environmental
sustainability.]

[Paragraph 3 - Quote from Founder Johnny Dubic or relevant leadership,
attributed by name and title.]

[Paragraph 4 - Additional context: relevant statistics, roadmap phase,
or partnership detail. Cite MEDIA_KIT.md statistics only where
confirmed; do not include unverified figures.]

About CeloHT
[Insert Company Boilerplate - see below]

Media Contact
[Name, title, email - see Media Contact Information]

###
```

**Usage note:** This template is provided for CeloHT's own announcements and for journalists constructing derivative coverage. Do not insert token, investment, or fundraising language inconsistent with the No Token Policy.

---

## Project Fact Sheet

| Field | Detail |
|---|---|
| Name | CeloHT |
| Type | Community-governed Web3 initiative |
| Headquarters | Léogâne, Haiti |
| Founder | Johnny Dubic |
| Blockchain | Celo |
| Assets used | USDm, CELO |
| Native token | None |
| Supported wallet | Valora, Minipay, WalletConnect|
| License | Apache 2.0 |
| Core pillars | Education, Agent Network, Reforestation |
| Governance | Governance Council, Working Groups, Maintainers, and Contributors |
| Current roadmap phase | Foundation documentation published; operational milestones remain planned or in progress |
| Website | celoht.com |
| GitHub | github.com/Celo-HaiTi/CeloHT |
| Primary contact | contact@celoht.com |

For the full statistics table and community impact figures, see MEDIA_KIT.md, Statistics and Community Impact sections.

---

## Company Boilerplate

**Standard (short):**
CeloHT is a community-governed Web3 initiative headquartered in Léogâne, Haiti, focused on financial inclusion, blockchain education, and environmental sustainability. Built on the Celo blockchain, CeloHT operates without a native token and is governed through a transparent, community-led structure. Learn more at celoht.com.

**Extended:**
CeloHT is a community-governed Web3 initiative headquartered in Léogâne, Haiti, built on the Celo blockchain. Its work is organized around three pillars: Education, an Agent Network, and Reforestation. CeloHT maintains a strict No Token Policy - it has no native token and does not conduct token sales, presales, or staking - and is governed through a Governance Council, Working Groups, Maintainers, and Contributors. CeloHT's code and documentation are open source under the Apache 2.0 license. Learn more at celoht.com or github.com/Celo-HaiTi/CeloHT.

---

## Founder Biography

**Johnny Dubic**, Founder, CeloHT

Johnny Dubic founded CeloHT in Léogâne, Haiti, to build a community-governed initiative addressing financial inclusion, blockchain education, and environmental sustainability. CeloHT's governance model intentionally limits founder authority, distributing decision-making across a Founder, Governance Council, and Contributors.

Extended professional background and headshot: TBD. For governance context, see MEDIA_KIT.md, Founder Biography and Governance sections.

---

## Leadership Profiles

| Role | Name | Profile |
|---|---|---|
| Founder | Johnny Dubic | See above |
| Founder | TBD | TBD |
| Governance Council | TBD | Council composition maintained in GOVERNANCE.md |

---

## Official Quotes

**On CeloHT's mission:**
No approved quotation is currently published. Obtain written confirmation before attributing a statement to a CeloHT representative.

**On governance:**
No approved quotation is currently published. Obtain written confirmation before publication.

**On the No Token Policy:**
Use the factual non-token statement in `LEGAL_STATUS.md`.

All quotes must be confirmed with the named speaker or CeloHT press contact prior to publication.

---

## Key Messages

1. CeloHT is a community-governed initiative, not a founder-controlled company.
2. CeloHT has no native token and does not solicit investment.
3. CeloHT's work spans three concrete pillars: Education, Agent Network, Reforestation.
4. CeloHT is built on the established Celo blockchain, not a proprietary or speculative chain.
5. CeloHT's code, documentation, and governance are fully open source under Apache 2.0.
6. CeloHT's Reforestation program is in pilot phase and not yet fully operational - this should be reflected accurately in any coverage.

---

## Media FAQs

**Is CeloHT affiliated with the Celo Foundation?**
CeloHT is built on the Celo blockchain but is a distinct, independently governed initiative. Formal affiliation status: TBD (confirm before publication).

**Can journalists quote CeloHT statistics directly?**
Only statistics confirmed in official documentation (MEDIA_KIT.md, ROADMAP.md, or direct confirmation from contact@celoht.com) should be cited. Do not extrapolate or estimate figures.

**Does CeloHT have a token journalists should be aware of?**
No. CeloHT has no native token. Any third-party token claiming affiliation with CeloHT is not authorized - see LEGAL_STATUS.md.

**Who can confirm quotes or statistics for a story on deadline?**
contact@celoht.com - see [Media Contact Information](#media-contact-information) for response expectations.

For general organizational FAQs (mission, programs, partnerships), see MEDIA_KIT.md, Frequently Asked Questions.

---

## Press Photos

Official press photography (founder portrait, leadership photos, program/field photography): TBD. Requests for press photography should be directed to contact@celoht.com. Until formal press photography is published, journalists should request current-available imagery directly rather than sourcing unofficial images.

---

## Logo Downloads

| Variant | File Location |
|---|---|
| Primary (yellow background) | `assets/branding/` - CeloHT Brand repository |
| Transparent | `assets/branding/` - CeloHT Brand repository |
| Favicon / icon-only | `assets/branding/` - CeloHT Brand repository |

Full logo specifications, exclusion zones, and color values are documented in BRAND_GUIDE.md. See also MEDIA_KIT.md, Logo System.

---

## Brand Assets

Complete brand assets - color system, typography, and visual identity guidelines - are maintained in the CeloHT Brand repository and summarized in MEDIA_KIT.md, Brand Identity through Visual Identity sections. Journalists requiring source files should contact contact@celoht.com or use the current Celo-HaiTi organization links.

---

## Screenshots and Product Images

Website and product interface screenshots: TBD. Once published, screenshots will be made available via the Brand or primary repository and linked here. Requests in the interim should be directed to contact@celoht.com.

---

## Event Photos

TBD. No official event photography is confirmed for publication at this time. This section will be updated as CeloHT participates in or hosts public events.

---

## Media Contact Information

| Purpose | Contact | Notes |
|---|---|---|
| Press inquiries | contact@celoht.com | Primary point of contact for journalists |
| Alternate contact | celoht3@gmail.com | Secondary channel |
| Partnership-related press | partnerships@celoht.com | For coverage involving partners such as FreClean |
| Security-related disclosures | security@celoht.com | Not for general press use |

**Expected response time:** TBD. Journalists on tight deadlines should note their timeframe explicitly in the subject line of their inquiry.

---

## Interview Guidelines

- Direct interview requests to contact@celoht.com with outlet name, topic, format, and desired timeframe.
- Specify whether the request concerns the Founder, a program lead, or a general organizational spokesperson.
- Allow reasonable lead time; CeloHT is a community-governed organization and some responses may require Governance Council or Founder input.
- Fact-check statistics and quotes with contact@celoht.com prior to publication.
- Do not frame interview questions or coverage in terms of investment returns, token value, or speculation - CeloHT representatives will decline to answer questions framed this way, consistent with the No Token Policy.

---

## Press Coverage Guidelines

When reporting on CeloHT, journalists are asked to:

- Accurately describe CeloHT as a community-governed initiative, not a company controlled by a single founder.
- Avoid describing CeloHT programs, USDm/CELO usage, or the Agent Network as investment products.
- Clearly state when a program (such as Reforestation) is in a pilot or design phase rather than fully operational.
- Attribute statistics to their source (MEDIA_KIT.md, ROADMAP.md, or direct CeloHT confirmation) and avoid presenting unconfirmed figures as fact.
- Direct fact-checking inquiries to contact@celoht.com prior to publication.

---

## Speaking Topics

CeloHT representatives are available to speak on:

- Financial inclusion through blockchain-based tools
- Community governance models for open-source and Web3 initiatives
- Blockchain education program design
- Environmental sustainability programs in emerging markets
- Building without a native token: an alternative model for Web3 initiatives

Requests should be directed to contact@celoht.com with event details, audience, and proposed date. See also MEDIA_KIT.md, Speaking Engagements.

---

## News Announcements

TBD. This section will list official CeloHT news announcements (program launches, partnership confirmations, roadmap milestones) as they are published, with links to the corresponding press release.

---

## Awards and Recognition

TBD. No awards or formal recognitions are confirmed for publication at this time. See also MEDIA_KIT.md, Awards.

---

## Downloadable Resources

| Resource | Location |
|---|---|
| Media Kit | MEDIA_KIT.md |
| Whitepaper | WHITEPAPER.md |
| Roadmap | ROADMAP.md |
| Governance charter | GOVERNANCE.md |
| No Token Policy | LEGAL_STATUS.md |
| Legal status summary | LEGAL_STATUS.md |
| Brand assets | Brand repository, `assets/branding/` |
| This Press Kit | PRESS_KIT.md |

---

## Citation Guidelines

When citing CeloHT in an article, report, or academic work:

- Cite CeloHT as the source organization, with a link to celoht.com or the relevant GitHub repository.
- Where citing a specific document (e.g., WHITEPAPER.md, ROADMAP.md), name the document explicitly.
- Do not attribute statistics or quotes to CeloHT that have not been confirmed via contact@celoht.com.

---

## Attribution Requirements

Any reproduction of CeloHT brand assets, quotes, or documentation excerpts must include clear attribution to CeloHT, with a link to celoht.com or the current Celo-HaiTi organization where feasible. Attribution does not imply endorsement of the publication or its content by CeloHT.

---

## Usage Permissions

Press and editorial use of CeloHT logos, quotes, and factual content from this Press Kit is permitted for accurate, good-faith news coverage without prior written permission, subject to the [Press Coverage Guidelines](#press-coverage-guidelines) above. Commercial, promotional, or non-editorial use requires prior written permission from contact@celoht.com. Full brand asset terms are set out in MEDIA_KIT.md, Brand Asset License.

---

## Copyright

Copyright CeloHT. Documentation and code are released under the Apache 2.0 license unless otherwise noted. Brand assets are subject to the terms described in MEDIA_KIT.md, Brand Asset License. All trademarks, logos, and brand elements referenced herein remain the property of their respective owners.

---

## Legal Disclaimer

CeloHT is not a financial institution, investment adviser, or securities issuer. Nothing in this Press Kit constitutes financial, investment, legal, or tax advice, or an offer or solicitation to buy or sell any asset. CeloHT has no native token and does not conduct token sales, ICOs, presales, or staking programs. For full legal status details, refer to LEGAL_STATUS.md. This document is provided for informational purposes and is subject to updates; the most current version supersedes any prior version.

---

*End of PRESS_KIT.md*
