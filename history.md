# History

## Origin

CeloHT started in Léogâne, Haiti, as a direct response to a simple reality:
a lot of people in rural and semi-rural communities don't have easy access
to traditional banking, even as mobile phone use keeps growing.

The founder identified Celo as a good fit because of its low transaction
fees and mobile-first design its native token, CELO, and its stablecoin,
USDm, are both accessible through the Valora wallet.

## Key Milestones

- **Early groundwork**: research and vision work for both CeloHT and
  FreClean, two complementary initiatives one centered on financial
  inclusion and blockchain, the other on professional cleaning services and
  local entrepreneurship.
- **Public presence**: CeloHT established a presence on Medium (@celoht3), X
  (@CeloHtOfficial), the Celo Forum, and the official Celo-HaiTi GitHub
  organization.
- **Community recognition**: CeloHT was featured in a "Founders" post on the
  Celo Forum in 2025, a notable moment of recognition within the
  broader Celo ecosystem.
- **Educational output**: development of a full library of Haitian Creole
  training material from a professional trading encyclopedia to a Web3/
  developer training book all built with open-source tools like Node.js
  and Python.
- **Flagship repository**: publication of this repository as the central
  governance and documentation hub for the entire ecosystem.

## What's Next

See [`roadmap.md`](roadmap.md) for what's coming.
